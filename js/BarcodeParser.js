...
var parseBarcode = (function () {
  ...
  function parseBarcode(barcode) {
    barcode = cleanFirstCodeString(barcode);
    var i = 0, // counter
      fncCharArr = ["\u0000", String.fromCharCode(29), "\u001d", "\x00"],
    ...
    function identifyAI(codestring) {
      ...
      function parseVariableLength(ai, title) {
        ...
        let arr = [];
        fncCharArr.forEach((chr) => {
          if (codestring.indexOf(chr) >= 0) {
            arr = codestring.split(chr);
          }
        });

        if (arr.length > 1) {
          let thisAi = arr.shift();
          elementToReturn.data = thisAi.slice(offSet, thisAi.length);
          codestringToReturn = arr.join(fncChar);
        } else {
      ...
      function parseVariableLengthMeasure(ai_stem, fourthNumber, title, unit) {
        ...
        if (arr.length > 1) {
          let thisAi = arr.shift();
          numberPart = thisAi.slice(offSet, thisAi.length);
          codestringToReturn = arr.join(fncChar);
        } else {
          elementToReturn.data = codestring.slice(offSet, codestringLength);
          codestringToReturn = "";
      ...
      function parseVariableLengthWithISONumbers(ai_stem, fourthNumber, title) {
        ...
        if (arr.length > 1) {
          let thisAi = arr.shift();
          isoPlusNumbers = thisAi.slice(offSet, thisAi.length);
          codestringToReturn = arr.join(fncChar);
        } else {
  ...
  const cleanFirstCodeString = (stringToClean) => {
    var fncCharArr = ["\u0000", String.fromCharCode(29), "\u001d", "\x00"];
    var firstChar = stringToClean.slice(0, 1);
    if (fncCharArr.includes(firstChar)) {
      stringToClean = stringToClean.slice(1, stringToClean.length);
    }
    //return fixFncChar(stringToClean);
    return stringToClean;
  };

  const fixFncChar = (stringToClean) => {
    const fixCharArr = ["\u0000", "\u001d", "\x00"];
    let replaceChar = String.fromCharCode(29);
    let arr = [];
    fixCharArr.forEach((fncChar) => {
      if (stringToClean.indexOf(fncChar) >= 0) {
        arr.push(stringToClean.split(fncChar));
      }
    });
    return arr.join(replaceChar);
  };
